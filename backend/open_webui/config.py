...
# 使用 glob 匹配所有以 .png 结尾的文件
for png_file in frontend_static_dir.glob("*.png"):
    try:
        # 复制文件到目标目录
        shutil.copyfile(png_file, STATIC_DIR / png_file.name)
        logging.info(f"Successfully copied {png_file.name} to {STATIC_DIR}")
    except Exception as e:
        logging.error(f"An error occurred while copying {png_file.name}: {e}")

# 使用 glob 匹配所有以 .webp 结尾的文件
for webp_file in frontend_static_dir.glob("*.webp"):
    try:
        # 复制文件到目标目录
        shutil.copyfile(webp_file, STATIC_DIR / webp_file.name)
        logging.info(f"Successfully copied {webp_file.name} to {STATIC_DIR}")
    except Exception as e:
        logging.error(f"An error occurred while copying {webp_file.name}: {e}")
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (Ibatch AI + Ollama)
